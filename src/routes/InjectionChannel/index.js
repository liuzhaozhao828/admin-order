...
class InjectionChannel extends React.Component {
  state = {
    visible: false,
    record: {}
  }
  ...
  getList=()=>{
    request('/admin/channel/getList', {type:['injection']}).then(({data: {code, msg, data={}}}) => {
          ...
          </Button>
          {
            ...
            <Button type='primary' size='small' onClick={()=>{
              Modal.confirm({
                ...
                cancelText: '取消',
                onOk: ()=>{
                  request('/admin/channel/action', { type: 'injection', action: 'stop', channelName: record.channelName, channelCode:record.channelCode }).then(({data: {code}})=>{
                    if(code==='000000'){
          ...
          }
          {
            ...
            <Button type='primary' size='small' onClick={()=>{
              Modal.confirm({
                ...
                cancelText: '取消',
                onOk: ()=>{
                  request('/admin/channel/action', { type: 'injection', action: 'use', channelName: record.channelName, channelCode:record.channelCode }).then(({data: {code}})=>{
                    if(code==='000000'){
...
export default InjectionChannel
